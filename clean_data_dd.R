...
date_recorded_month <- factor(format.Date(as.Date(train$date_recorded), format = "%b"))
...
date_recorded_month <- factor(format.Date(as.Date(test$date_recorded), "%b"))
...
train <- left_join(train, read.csv('./Data/train_labels.csv'), by = 'id')
