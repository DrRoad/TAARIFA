...
temp1 <- glm(status_group ~ longitude + latitude, data=train, family=binomial)
temp2 <- glm(status_group ~ bs(longitude) + bs(latitude), data=train, family=binomial)

plot.roc(train$status_group, predict(temp1, newdata=train, type="response"))
plot.roc(train$status_group, predict(temp2, newdata=train, type="response"))

# Spline Interaction
temp3 <- glm(status_group ~ bs(longitude) + bs(latitude) + bs(longitude)*bs(latitude), 
             data=train, family=binomial)
plot.roc(train$status_group, predict(temp3, newdata=train, type="response"))

# 3 top variables interactions
temp4 <- glm(status_group ~ (bs(longitude) + bs(latitude) + quantity)^2, 
             data=train, family=binomial)
plot.roc(train$status_group, predict(temp4, newdata=train, type="response"))
```
