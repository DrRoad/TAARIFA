...
(For future reference, to use more than 4gb memory we must be using 64 bit R and reset memory limit using the function memory.limit() to take advantage of a high memory machine)
...
ggplot(data=train) + 
  geom_point(aes(x=longitude, y=latitude, 
                  color=status_group, alpha=0.2, size=1), size=1) +
...
# Sample and split into training and classification variables
knn.train <- sample_frac(train, .8)
knn.test <- setdiff(train, knn.train)

knn.trainlabel <- knn.train[,"status_group"]
knn.train <- select(knn.train, longitude, latitude, gps_height)
knn.testlabel <- knn.test[,"status_group"]
knn.test <- select(knn.test, longitude, latitude, gps_height)
